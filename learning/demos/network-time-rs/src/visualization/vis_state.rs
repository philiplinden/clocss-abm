use crate::model::{NetNode, NetworkState};
...
impl VisualizationState<NetworkState> for VisState {
    fn on_init(
        &self,
        ...
        NetworkState::init_network_graphics(state, commands)
    }
    ...
    fn get_agent(
        ...
        let state = state.as_any().downcast_ref::<NetworkState>().unwrap();
